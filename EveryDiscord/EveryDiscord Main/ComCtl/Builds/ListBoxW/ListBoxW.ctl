...
P.X = X
P.Y = Y
If ListBoxHandle <> NULL_PTR Then MapWindowPoints UserControl.hWnd, ListBoxHandle, P, 1
...
ElseIf ListBoxHandle <> NULL_PTR Then
    ...
    ListBoxDragIndex = LBItemFromPt(ListBoxHandle, XY, 0) + 1
End If
...
If ListBoxHandle <> NULL_PTR Then
    AllowDropFiles = CBool((GetWindowLong(ListBoxHandle, GWL_EXSTYLE) And WS_EX_ACCEPTFILES) <> 0)
Else
...
If ListBoxHandle <> NULL_PTR Then DragAcceptFiles ListBoxHandle, IIf(PropAllowDropFiles = True, 1, 0)
...
If ListBoxHandle <> NULL_PTR Then
    ...
    CopyMemory ByVal VarPtr(XY), ByVal VarPtr(P), 8
    HitTest = LBItemFromPt(ListBoxHandle, XY, 0)
End If
End Function
...
If ListBoxHandle <> NULL_PTR Then
    ...
    ClientToScreen ListBoxHandle, P
    CopyMemory ByVal VarPtr(XY), ByVal VarPtr(P), 8
    Index = LBItemFromPt(ListBoxHandle, XY, 0)
    If Index > -1 Then
        Dim RC As RECT
        SendMessage ListBoxHandle, LB_GETITEMRECT, Index, ByVal VarPtr(RC)
...
Select Case wMsg
    ...
    Case WM_DROPFILES
        If wParam <> NULL_PTR Then
            ...
            If FileCount > 0 Then
                Dim FileList() As String, iFile As Long, FileBuffer As String, P3 As POINTAPI
                ...
                Next iFile
                DragQueryPoint wParam, P3
                RaiseEvent DropFiles(FileList(), UserControl.ScaleX(P3.X, vbPixels, vbContainerPosition), UserControl.ScaleY(P3.Y, vbPixels, vbContainerPosition))
            End If
            DragFinish wParam
        End If
        WindowProcControl = 0
    ...
    Case WM_CONTEXTMENU
        If wParam = ListBoxHandle Then
            ...
            Else
                ScreenToClient ListBoxHandle, P4
                RaiseEvent ContextMenu(UserControl.ScaleX(P4.X, vbPixels, vbContainerPosition), UserControl.ScaleY(P4.Y, vbPixels, vbContainerPosition))
            End If
        End If
...
Select Case wMsg
    ...
    Case LB_SETTABSTOPS
        If WindowProcControl <> 0 Then
            If wParam > 0 Then
                ListBoxTabPositions = CLng(wParam)
